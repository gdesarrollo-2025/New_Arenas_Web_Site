...
export async function phraseToUrl(input) {
    ...
    for (const type of propertyTypes.sort((a, b) => b.name.length - a.name.length)) {
        ...
        if (regex.test(text)) {
            result.propertyType.push(type.name);
            text = text.replace(regex, "");
        }
    }

    url +=`/${result.propertyType.join("-y-")}`
...
export async function urlToQuery(slug) {
    ...
    //inicializa el resultado en json
    const result = {
        location: [],
        neighborhood: [],
        propertyType: [],
    ...
    const [citiesData, neighborhoodsData, propertiesData] = await Promise.all([
        fetch("http://localhost:3000/api/domus-cities").then((r) => r.json()),
        fetch("http://localhost:3000/api/domus-neighborhoods").then((r) => r.json()),
        fetch("http://localhost:3000/api/domus-types").then((r) => r.json()),
    ]);
    ...
    for (const type of propertyTypes.sort((a, b) => b.name.length - a.name.length)) {
        const regex = new RegExp(`\\b${type.name.toLowerCase()}\\b`, "i");
        if (regex.test(text)) {
            result.propertyType.push(type.code);
            text = text.replace(regex, "");
        }
    }
    ...
    for (const n of neighborhoods) {
        const regex = new RegExp(`\\b${n.name}\\b`, "i");
        if (regex.test(text)) {
            result.neighborhood.push(n.code);
            if (!result.location.length) result.location.append(n.city_code);
            text = text.replace(regex, "");
            break;
        }
    }
