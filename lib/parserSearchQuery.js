...
export default async function parseSearchQuery(input) {
    ...
    if (bathMatch) {result.bathrooms = parseInt(bathMatch[1]); text = text.replace(bathMatch,"");};
    ...
    result['q'] = text.trim();
    console.log("Parsed search query:", result);
    return result;
}