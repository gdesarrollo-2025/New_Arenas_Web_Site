...
export default function FilterSidebar() {
  ...
  useEffect(() => {
    ...
    if (type) newFilters.propertyType = Array.isArray(type) ? type : type.split(',');
    if (biz) newFilters.bizType = biz;
    ...
    if (bedrooms) newFilters.bedrooms = Array.isArray(bedrooms) ? bedrooms : [bedrooms];
    if (bathrooms) newFilters.bathrooms = Array.isArray(bathrooms) ? bathrooms : [bathrooms];
    
    // Estrato
    if (stratum) newFilters.stratum = stratum;
    
    // Área
    if (minarea) newFilters.minArea = minarea;
    if (maxarea) newFilters.maxArea = maxarea;
